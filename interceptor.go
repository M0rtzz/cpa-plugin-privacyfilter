...
func (p *privacyFilterPlugin) interceptRequest(req pluginapi.RequestInterceptRequest) (pluginapi.RequestInterceptResponse, error) {
	...
	if counts.Han*100 > counts.Total*int64(p.cfg.MaxHanPercent) {
		message := fmt.Sprintf("本轮指令的汉字占比超过 %d%%，请翻译成英语后重新发送。可以用英语要求模型用中文回复。\nPlease reply in Chinese, but write all questions that require my answer and their answer options in English.", p.cfg.MaxHanPercent)
